...
function renderContentList() {
    ...
    const contents = JSON.parse(localStorage.getItem(CONTENT_STORAGE_KEY) || '[]');
    ...
    contentList.innerHTML = contents.map(content => `
        <div class="content-item">
            ...
            <button class="Documents-btn" 
                ...
                data-story="${encodeURIComponent(content.successStory)}">
                ...
                <p class="text">Share Success Story</p>
            </button>
        </div>
    `).join('');
}
...
function shareViaEmail(button) {
    currentStoryData = {
        ...
        story: decodeURIComponent(button.getAttribute('data-story'))
    };
...
function sendEmail() {
    ...
    const subject = encodeURIComponent(`Success Story: ${currentStoryData.title}`);
    const body = encodeURIComponent(`Check out this amazing success story:\n\n${currentStoryData.story}`);
